...
(function(){
  ...
  const groupGrade=id=>db.groups.find(g=>g.id===id)?.grade||String(id||'').charAt(0);
  function renderIncidencias(){
    const rows=[...(db.incidents||[])].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))).map(x=>{
      ...
      return `<tr><td>${esc2(x.date||'')}</td><td><b>${esc2(st?.name||'Alumno eliminado')}</b></td><td>${esc2(groupName(gid))}</td><td>${esc2(x.type||'')}</td><td>${esc2(x.severity||x.gravedad||'')}</td><td>${esc2(x.followup||x.seguimiento||'')}</td><td><button class="btn" onclick="openIncidentModal('${x.id}')">Editar</button> <button class="btn danger" onclick="deleteIncident('${x.id}')">Eliminar</button></td></tr>`;
    }).join('');
    document.getElementById('content').innerHTML=`<div class="toolbar"><button class="btn primary" onclick="openIncidentModal()">＋ Nueva incidencia</button></div><div class="card"><div class="table-wrap"><table class="table"><thead><tr><th>Fecha</th><th>Alumno</th><th>Grupo</th><th>Tipo</th><th>Gravedad</th><th>Seguimiento</th><th>Acciones</th></tr></thead><tbody>${rows||'<tr><td colspan="7" class="empty">No hay incidencias registradas.</td></tr>'}</tbody></table></div></div>`;
  }
  function openIncidentModal(id){
    ...
    document.getElementById('pdIncGroup').onchange=fillStudents; fillStudents();
