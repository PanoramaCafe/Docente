...
(function(){
  ...
  function inferSubject(groupId,subjectId){
    const grade=gradeFromGroup(groupId);
    if(subjectId&&window.PD_CONFIG?.subjectAllowed(subjectId,grade))return subjectId;
    if(grade==='2'||grade==='3')return 'vida23';
  ...
  function getOrCreate({groupId,title,date,subjectId,points=10,source='trabajo'}){
    ...
    subjectId=inferSubject(groupId,subjectId);
    let a=find(groupId,title,date,subjectId);
    if(a){if(source==='impresion')a.source=a.source||'impresion';return a}
    a={id:uid('act'),groupId,title:String(title).trim(),date,subjectId,points:Number(points)||10,source};
  ...
  window.PD_WORKS={find,getOrCreate,linkPrint,inferSubject:(groupId,subjectId)=>inferSubject(groupId,subjectId)};
})();
